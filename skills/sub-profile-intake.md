...
description: (career-path-advisor) Capture current role, skills, constraints, values and target horizon to anchor the roadmap.
---

# Sub-skill: profile-intake

## Purpose
Capture current role, skills, constraints, values and target horizon to anchor the roadmap.

## When the Harness Calls This
Stage matching `sub-profile-intake` in the `career-path-advisor` main workflow.
